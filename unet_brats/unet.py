...
def img_warp(img, mask, theta_max=15, offset_max=0, scale_max=1.1, shear_max=0.1):
    ...
    # Scale/zoom
    # Z = np.random.uniform(1 - scale_max, 1 + scale_max, 3)
...
def get_weights(label):
    """Calculate class weights according to their frequency in label map"""
    counts, _ = np.histogram(label, bins=np.arange(0, 4+1, dtype='int32'))
    counts = counts.astype('float32')
    counts[counts==0] = float('nan')
    weights = np.nansum(counts) / counts
    weights = weights / np.nanmax(weights)
    weights[np.isnan(weights)] = 0
    return weights 

def put_weights(label, weights_avg):
    """Generate weighting map from label map and given class weights"""
    label = label.asnumpy()
    weights = np.zeros(label.shape)
    for i in range(len(weights_avg)):
        weights[label==i] = weights_avg[i]
